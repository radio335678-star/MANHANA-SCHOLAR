...
export const MAX_THESIS_PAGES = 500;
...
export function totalTargetPages(
  sections: Array<{ targetPages?: number | null; minPages?: number | null; maxPages?: number | null }>,
): number {
  return sections.reduce((sum, s) => {
    const pages = s.targetPages ?? s.minPages ?? 1;
    return sum + pages;
  }, 0);
}
